...
export const addNewTaskApi=async(formData)=>{
    ...
    const response=await axios.post("http://localhost:5000/api/task/add-new-task",formData)

    return response?.data;

}
export const getAllTasksApi=async(getCurrentUserId)=>{
    const response=await axios.get(`http://localhost:5000/api/task/get-all-tasks-by-userid/${getCurrentUserId}`)
    return response?.data;

    
}
export const deleteTaskApi=async(getCurrentTaskId)=>{
    const response=await axios.delete(`http://localhost:5000/api/task/delete-task/${getCurrentTaskId}`)
    return response?.data;
...
export const updateTaskApi=async(formData)=>{
     ...
     const response=await axios.put(`http://localhost:5000/api/task/update-task/`,
        formData
     );
    return response?.data;
